...
import net.majorkernelpanic.spydroid.api.CustomRtspClient;
...
public class SpydroidActivity extends FragmentActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (findViewById(R.id.handset_pager) != null) {
			...
			setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);//手机屏幕朝向，竖屏显示
			...
			setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);//手机屏幕朝向，横屏显示
	...
	public void onStart() {
		...
		bindService(new Intent(this,CustomRtspClient.class), mRtspClientConnection, Context.BIND_AUTO_CREATE);
	}

	@Override
	public void onStop() {
		super.onStop();
		//当isHeld()是true，WakeLock应该只能被释放
		...
		if (mRtspClient != null) mRtspClient.removeCallbackListener(mRtspClientCallbackListener);
		unbindService(mRtspClientConnection);
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.options:
			// QualityListActivity开始,用户可以修改流质量
	...
	private void quitSpydroid() {
		...
		this.stopService(new Intent(this,CustomRtspClient.class));
		// 回到home菜单
		finish();
	}
	
	private ServiceConnection mRtspClientConnection = new ServiceConnection() {

		@Override
		public void onServiceConnected(ComponentName name, IBinder service) {
			mRtspClient = (CustomRtspClient) ((RtspClient.LocalBinder)service).getService();
			mRtspClient.addCallbackListener(mRtspClientCallbackListener);
	...
	private RtspClient.CallbackListener mRtspClientCallbackListener = new RtspClient.CallbackListener() {

		@Override
		public void onError(RtspClient server, Exception e, int error) {
			// 我们提醒用户,端口已经被另一个应用程序使用。
			if (error == RtspClient.ERROR_BIND_FAILED) {
